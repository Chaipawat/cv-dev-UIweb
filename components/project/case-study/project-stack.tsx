...
export default function ProjectStack({ project }: { project: Project }) {
  return (
    <div>
      <ul className="m-0 flex list-none flex-wrap items-baseline gap-x-[0.35em] gap-y-1 p-0 font-display text-[clamp(30px,5.4vw,80px)] font-extrabold uppercase leading-[1] tracking-[-0.05em] text-foreground">
      ...
      {project.backendFamiliarity ? (
        <div className="mt-[clamp(32px,4vw,56px)] grid grid-cols-1 gap-2 border-t border-border pt-5 md:grid-cols-12 md:gap-8">
          <span className="font-mono text-[10px] tracking-[0.2em] text-foreground-muted md:col-span-3">
            BACKEND FAMILIARITY
          </span>
