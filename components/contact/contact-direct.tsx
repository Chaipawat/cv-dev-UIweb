...
export default function ContactDirect() {
  return (
    <div className="rounded-lg border border-border p-6 md:p-8">
      <div className="pb-5 font-mono text-[10px] tracking-[0.2em] text-foreground-muted">GET IN TOUCH</div>
        ...
        {CONTACT_LINKS.map((c) => {
          ...
          const content = (
            <>
              ...
              <div>
                <div className="font-mono text-[10px] tracking-[0.18em] text-foreground-muted">{c.key}</div>
                <div
                  className={cn(
                    "mt-2 min-w-0 break-all text-[15px] leading-snug",
                    c.href ? "text-foreground" : "font-mono text-foreground-muted"
                  )}
                >
                  {c.value}
                </div>
