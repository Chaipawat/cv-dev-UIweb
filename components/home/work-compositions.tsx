...
import ProjectCover, {
  hasProjectCover,
} from "@/components/project/project-cover";
import ZonepangEcosystem from "@/components/home/zonepang-ecosystem";
import MiniGameComposition from "@/components/home/minigame-composition";
import BroadpangExtensionComposition from "@/components/home/broadpang-extension-composition";
import DevPathComposition from "@/components/home/devpath-composition";
import KumtoneComposition, { KumtoneStoreEvidence } from "@/components/home/kumtone-composition";
...
function Title({
  ...
  return (
    <h3
      data-reveal="clip"
      className={cn(
        "m-0 font-display uppercase tracking-[-0.01em] text-foreground",
        className,
      )}
    >
      <Link
        href={href(project)}
        className="group inline transition-colors duration-[320ms] hover:text-accent"
      >
  ...
  return (
    ...
    >
      {rows.map((r) => (
        ...
        >
          <dt className="font-mono text-[10px] tracking-[0.2em] text-foreground-secondary">
            {r.k}
          </dt>
          <dd className="m-0 mt-1.5 text-[14px] leading-[1.5] text-foreground-secondary">
            {r.v}
          </dd>
        </div>
      ))}
  ...
  return (
    <article>
      ...
      <h3 className="m-0 mt-[clamp(20px,3vw,40px)] font-display text-[clamp(38px,11.5vw,176px)] uppercase leading-[0.84] tracking-[-0.01em] text-foreground">
        <Link
          href={href(project)}
          className="group block transition-colors duration-[320ms] hover:text-accent"
        >
          <span data-reveal="clip" className="block origin-left">
            {first}
          </span>
          <span
            data-reveal="clip"
            className="block origin-left pl-[clamp(20px,14vw,260px)]"
          >
            {rest.join(" ")}
            <span
              aria-hidden="true"
      ...
      <div className="mt-[clamp(40px,5vw,64px)] grid grid-cols-1 gap-8 lg:grid-cols-12">
        <p
          data-reveal="fade"
          className="m-0 max-w-[460px] text-[16px] leading-[1.6] text-foreground-secondary lg:col-span-4"
        >
          {project.description}
        </p>
...
export function AsymmetricComposition({ project }: CompositionProps) {
  ...
  return (
    ...
    >
      {cover ? (
        project.slug === "zonepang-platform" ? (
          <div className="lg:col-span-8">
            <ZonepangEcosystem project={project} />
          </div>
        ) : (
        ...
        <ul className="order-last m-0 hidden list-none flex-col justify-end border-l border-border p-0 pl-5 lg:order-none lg:col-span-3 lg:flex">
          {project.features.map((f) => (
            <li
              key={f}
              className="py-1.5 font-mono text-[11px] uppercase tracking-[0.16em] text-foreground-secondary"
            >
              {f}
            </li>
          ))}
      ...
      >
        ...
        <Title
          ...
          className={cn(
            "mt-5 leading-[0.88]",
            cover
              ? "text-[clamp(40px,5.6vw,84px)]"
              : "text-[clamp(44px,9vw,140px)] lg:text-right",
          )}
        />
        <p
          data-reveal="fade"
          className="m-0 mt-6 max-w-[460px] text-[15.5px] leading-[1.6] text-foreground-secondary lg:self-end"
        >
          {project.description}
        </p>
        <Meta
          project={project}
          layout="stack"
          className={cn("mt-8", !cover && "lg:w-[60%] lg:self-end")}
        />
      </div>
    </article>
  );
  ...
  return (
    ...
    >
      <div className="md:col-span-5">
        ...
        <p
          data-reveal="fade"
          className="m-0 mt-6 max-w-[420px] text-[15.5px] leading-[1.6] text-foreground-secondary"
        >
          {project.description}
        </p>
        <Meta project={project} layout="stack" className="mt-8" />
        {project.slug === "kumtone" ? <KumtoneStoreEvidence project={project} /> : null}
      </div>
      <div className="md:col-span-6 md:col-start-7">
        {cover ? (
          project.slug === "kumtone" ? (
            <KumtoneComposition project={project} />
          ) : (
          ...
          <ul className="m-0 list-none border-t border-border p-0 md:mt-[clamp(40px,8vw,120px)]">
            {features.map((f, i) => (
              <li
                key={f}
                data-reveal="fade"
                className="flex items-baseline gap-4 border-b border-border py-[clamp(10px,1.4vw,18px)] font-serif text-[clamp(26px,3.4vw,48px)] italic leading-[1.05] text-foreground"
              >
    ...
    <article>
      <div className="grid grid-cols-1 gap-[clamp(28px,4vw,56px)] lg:grid-cols-12 lg:gap-8">
        ...
        {project.context ? (
          <blockquote
            data-reveal="fade"
            className="m-0 border-l border-accent-border pl-6 font-serif text-[clamp(22px,2.6vw,36px)] italic leading-[1.2] text-foreground-secondary lg:col-span-5 lg:col-start-8 lg:mt-12"
          >
            {project.context}
          </blockquote>
        ) : null}
      ...
      {cover ? (
        project.slug === "broadpang-extension" ? (
          <BroadpangExtensionComposition project={project} />
        ) : (
        <CoverLink
          project={project}
          className="mt-[clamp(32px,5vw,64px)] lg:ml-[16%]"
        >
          <ProjectCover project={project} composition="browser" />
        </CoverLink>
        )
      ...
      <div className="mt-[clamp(28px,4vw,48px)] grid grid-cols-1 gap-8 lg:grid-cols-12">
        <p
          data-reveal="clip"
          className="m-0 font-mono text-[clamp(20px,2.8vw,40px)] font-medium uppercase leading-[1.1] tracking-[-0.02em] text-accent-text lg:col-span-5"
        >
          {project.stack.join(" / ")}
        </p>
        <div className="lg:col-span-6 lg:col-start-7">
          <p
            data-reveal="fade"
            className="m-0 text-[15.5px] leading-[1.6] text-foreground-secondary"
          >
            {project.description}
          </p>
  ...
  return (
    ...
    >
      ...
      <div className="md:col-span-5">
        <Title
          project={project}
          className="text-[clamp(32px,4.4vw,64px)] leading-[0.92]"
        />
        <p
          data-reveal="fade"
          className="m-0 mt-5 max-w-[440px] text-[15px] leading-[1.6] text-foreground-secondary"
        >
          {project.description}
        </p>
        {cover && isMiniGame ? <MiniGameComposition project={project} /> : null}
      </div>
  ...
  return (
    ...
    >
      <div className="lg:col-span-4">
        ...
        <p
          data-reveal="fade"
          className="m-0 mt-6 max-w-[380px] text-[15.5px] leading-[1.6] text-foreground-secondary"
        >
          {project.description}
        </p>
        <Meta project={project} layout="stack" className="mt-8" />
      ...
      <div className="order-first lg:order-none lg:col-span-7 lg:col-start-6">
        <h3
          data-reveal="clip"
          className="m-0 font-serif text-[clamp(56px,14vw,216px)] font-normal italic leading-[0.85] tracking-[-0.02em] text-foreground lg:text-right"
        >
          <Link
            href={href(project)}
            className="transition-colors duration-[320ms] hover:text-accent"
          >
            {project.title}
            <span
              aria-hidden="true"
              className="ml-[0.1em] font-display text-[0.3em] not-italic text-foreground-muted"
            >
              →
            </span>
          </Link>
        </h3>
        {cover && project.slug !== "devpath" ? (
      ...
      {cover && project.slug === "devpath" ? (
        <div className="min-w-0 lg:col-span-12">
          <DevPathComposition project={project} />
        </div>
      ) : null}
    </article>
  );
