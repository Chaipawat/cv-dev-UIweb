...
import ProjectVisualStory from "@/components/project/project-visual-story";
import { portfolio } from "@/data/portfolio";
import { PROJECT_SLUGS, formatProjectPeriod, getProjectBySlug } from "@/data/projects";
import { getProjectMediaGroups } from "@/lib/project-media";
...
export default async function ProjectPage({ params }: PageProps<"/work/[slug]">) {
  ...
  const hasScreens = getProjectMediaGroups(project).length > 0;
  ...
  const candidates: (Section | null)[] = [
    ...
    project.highlights?.length || project.features?.length
      ...
      : null,
    hasScreens ? { label: "SCREENS", content: <ProjectVisualStory project={project} /> } : null,
